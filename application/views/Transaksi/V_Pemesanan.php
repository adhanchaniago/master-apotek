				...
				<div class="box box-primary">
					...
					<form action="" method="POST" id="register" role="form">
						<div class="box-body row">
							...
							<div class="col-lg-4">
								<div class="form-group">
									<label for="qty">Qty (Kemasan)</label>
						...
						<div class="box-footer">
							...
							<?php else : ?>
								<span class="col-lg-6" style="padding-left:0">
									<button type="button" id="Checkout" class="btn btn-block btn-success">Checkout</button>
								</span>
								<span class="col-lg-6" style="padding-right:0">
									<button type="button" id="Destroy" class="btn btn-block btn-danger">Hapus Pesanan</button>
								</span>
	...
	<script>
		$(document).ready(function(){
			...
			if(kode!="") {
				...
				$.ajax({
					type: "GET",
					url: "<?= base_url('pemesanan/get_detail/'.$this->uri->segment(3)) ?>",
					success: function(data){
				...
				$.ajax({
					type: "GET",
					url: "<?= base_url('pemesanan/get_status/'.$this->uri->segment(3)) ?>",
					success: function(data){
						var val = $.parseJSON(data);
						if(val.status) {
		...
		$(document).on('click','#Hapus',function() {
			...
			jQuery.ajax({
				type: "POST",
				url: "<?= base_url('pemesanan/hapus_data/'.$this->uri->segment(3)) ?>",
				dataType: 'json',
				data: {
						id_barang : id_barang
					},
		...
		$(document).on('click','#Simpan',function() {
			var id_pbf = $('select[name=id_pbf]').val();
			var id_sp = $("#id_sp").val();
			jQuery.ajax({
				type: "POST",
				...
				success: function(data) {
					window.location.href = "<?= base_url('laporan/pemesanan') ?>";
				}
			});
		});
		...
		$(document).on('click','#Checkout',function() {
			jQuery.ajax({
				type: "POST",
				url: "<?= base_url('pemesanan/checkout/'.$this->uri->segment(3)) ?>",
				dataType: 'json',
				success: function(data) {
					window.location.href = "<?= base_url('transaksi/pembelian/') ?>"+data.url;
				}
			});
		});
		function reload() {
