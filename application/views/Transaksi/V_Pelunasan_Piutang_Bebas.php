	...
	<script>
		$(document).ready(function(){
			var dtTable = $('#dtTable').DataTable({
				...
				"ajax": {
					"url": "<?= base_url('penjualan/list_data_piutang') ?>",
					"type": "POST"
				},
				//dom: 'Bfrtip',
				buttons: [ 'print' ],
			...
			var dtDetail = $('#dtDetail').DataTable({
				"processing": true,
				"ajax": {
					"url": "<?= base_url('penjualan/list_detail_piutang/null') ?>",
					"type": "POST"
				},
				"autoWidth": false,
				"info": false,
			...
			event.preventDefault();
				...
				Pace.track(function(){
					jQuery.ajax({
						type: "POST",
						url: "<?= base_url('penjualan/bayar_piutang/') ?>"+kode,
						dataType: 'json',
						data: {
								bayar : bayar
							},
		...
		$(document).on('click','#getdetail',function() {
			...
			Pace.track(function(){
				$('#dtDetail').DataTable().ajax.url('<?= base_url('penjualan/list_detail_piutang/') ?>'+kode).load();
			});
			$.ajax({
				type: "GET",
				url: "<?= base_url('penjualan/get_sisa_hutang/') ?>"+kode,
				success: function(data){
					if(data){
			...
			$.ajax({
				type: "GET",
				url: "<?= base_url('penjualan/get_bayar_hutang/') ?>"+kode,
				success: function(data){
					if(data){
						var opts = $.parseJSON(data);
